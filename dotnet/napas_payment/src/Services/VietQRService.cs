...
public class VietQRService : IVietQRService
{
     ...
     string accountNumber, string note, string currency, string countryCode)
    {
        ...
        if (string.IsNullOrEmpty(bankBIN) || string.IsNullOrEmpty(accountNumber))
            throw new ArgumentException("BankBIN and AccountNumber cannot be null or empty");
    ...
    private static string GenerateObject(ObjectDef[] defs, string prefixId, string id, Dictionary<string, string> contents)
    {
    ...
    private static string GenerateObject2(ObjectDef[]? defs, string prefixId, string id, Dictionary<string, string> contents)
    {
        ObjectDef? def = null;
        if (defs != null)
        {
            def = defs.FirstOrDefault(d => d.ID == id);
        }

        if (defs == null || defs.Length == 0)
        {
            def = new ObjectDef { ID = "--", Sub = GetDefaults() };
        }
        ...
        var contentValue = ToAscii(contents.GetValueOrDefault(prefixId + id, ""));
        if (string.IsNullOrEmpty(contentValue))
        {
            return "";
        }
    ...
    private static string GenerateObject3(ObjectDef[] defs, string prefixId, string id, Dictionary<string, string> contents)
    {
        if (string.IsNullOrEmpty(id))
        {
            // Root level - process all defaults
    ...
    private static ObjectDef[] GetDefaults2()
    {
        return new[]
        {
            new ObjectDef { ID = "00", Required = "M" },
            new ObjectDef { ID = "01", Required = "M" },
            new ObjectDef { ID = "38", MaxLen = 99, Required = "M" },
            new ObjectDef { ID = "52", Required = "O" },
            new ObjectDef { ID = "53", Required = "M" },
            new ObjectDef { ID = "54", MaxLen = 13, Required = "C" },
            new ObjectDef { ID = "55", Required = "O" },
            new ObjectDef { ID = "56", MaxLen = 13, Required = "C" },
            new ObjectDef { ID = "57", MaxLen = 5, Required = "C" },
            new ObjectDef { ID = "58", Required = "M" },
            new ObjectDef { ID = "59", MaxLen = 25, Required = "O" },
            new ObjectDef { ID = "60", MaxLen = 15, Required = "O" },
            new ObjectDef { ID = "51", MaxLen = 10, Required = "O" },
            new ObjectDef { ID = "62", MaxLen = 99, Required = "C" },
            new ObjectDef { ID = "64", MaxLen = 99, Required = "O" }
        };
    }
}
...
public class Bank
{
    ...
    public string AndroidBundle { get; set; } = string.Empty;
